                ...
                if has_update_date and has_create_date:
                    ...
                    for i in range(len(lines)):
                        if re.match(
                            ...
                            if (
                                ...
                                print(f"{os.path.relpath(item_path)} 日期已是最新\n")
                                break
                            else:
                                lines[i] = (
                                    f':material-clock-edit-outline:{{ title="修改日期" }} {modify_time}\n'
                                ...
                                f.writelines(lines)
                                print(
                                    f"{os.path.relpath(item_path)} 日期已修改，修改后的日期为：{modify_time}\n"
                                )
                                break
                    ...
                    for i in range(len(lines)):
                        if re.match(
                            ...
                            f.writelines(lines)
                            print(
                                f"{os.path.relpath(item_path)} 已覆盖原有修改日期数据，修改后的日期为：{modify_time}\n"
                            )
                            break
                else:
                    # 没有日期数据，添加日期数据
                    ...
                    f.seek(0)
                    f.writelines(lines)
                    print(f"{os.path.relpath(item_path)} 无日期，已添加\n")
